...
class MoveItPickAndPlaceDemo():
    def __init__(self):
        ...
        # 将目标位置设置为机器人的抓取目标位置
        grasps_pose = target_pose
        grasps_pose.pose.orientation.x = 0
        grasps_pose.pose.orientation.y = 0.707
        grasps_pose.pose.orientation.z = 0
        grasps_pose.pose.orientation.w = 0.707
        grasps_pose.pose.position.z += 0.01
        self.gripper_pose_pub.publish(grasps_pose)
                
        # 生成抓取姿态
        grasps = self.make_grasps(grasps_pose, [target_id])
        print("**************************\n")
        print(grasps)
    ...
    def make_grasps(self, initial_pose_stamped, allowed_touch_objects):
        ...
        # 设置期望的夹爪靠近、撤离目标的参数
        g.pre_grasp_approach = self.make_gripper_translation(0.01, 0.1, [0, 1, 0])
        g.post_grasp_retreat = self.make_gripper_translation(0.1, 0.2, [0, 0, 1.0])
